 ...
 * see watch-policy.ts), the CodeGG index would otherwise go stale until
 * the user runs `codegg sync` by hand. As an opt-in alternative, we can
 ...
 *
 * The hooks run `codegg sync` in the background so they never block git,
 * and are guarded by `command -v codegg` so they no-op cleanly when the
...
const MARKER_BEGIN = '# >>> codegg sync hook >>>';
const MARKER_END = '# <<< codegg sync hook <<<';
...
function markerBlock(): string {
  return [
    MARKER_BEGIN,
    '# Keeps the CodeGG index fresh while the live file watcher is off',
    '# (e.g. WSL2 /mnt drives). Runs in the background so it never blocks git.',
    '# Managed by codegg; remove with `codegg uninit` or delete this block.',
    'if command -v codegg >/dev/null 2>&1; then',
    '  ( codegg sync >/dev/null 2>&1 & ) >/dev/null 2>&1',
    'fi',
    MARKER_END,
  ].join('\n');
}
...
/**
 * Install (or update) the CodeGG sync hooks in a git repository.
...
/**
 * Remove the CodeGG sync hooks. Strips only our marker block; deletes the
...
/** Whether any CodeGG sync hook is currently installed. */
