#!/usr/bin/env bash
# Build the current branch and link it as the global `codegg` for
...
if [ "${1:-}" = "--undo" ]; then
  ...
  npm install -g "${PKG}"
  echo "done: global codegg -> $(command -v codegg)"
  exit 0
fi
...
LINKED=$(command -v codegg || echo "(not on PATH)")
echo
echo "✓ global codegg now points to this branch"
echo "  binary:  ${LINKED}"
echo "  branch:  ${BRANCH}"
