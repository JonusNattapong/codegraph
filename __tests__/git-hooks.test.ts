...
describe('git sync hooks', () => {
  ...
  beforeEach(() => {
    repo = fs.mkdtempSync(path.join(os.tmpdir(), 'codegg-githooks-'));
  });
  ...
  it('installs all default hooks, executable, invoking codegg sync', () => {
    gitInit(repo);
    ...
    for (const hook of DEFAULT_SYNC_HOOKS) {
      ...
      expect(body).toContain('codegg sync');
      expect(body).toContain('command -v codegg'); // no-op when not on PATH
  ...
  it('is idempotent — re-install does not duplicate the block', () => {
    ...
    const occurrences = body.split('# >>> codegg sync hook >>>').length - 1;
  ...
  it('remove keeps user content when the hook is shared', () => {
    ...
    expect(body).not.toContain('codegg sync');
  });
