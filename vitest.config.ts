...
export default defineConfig({
  test: {
     ...
     * mcp-roots) spawn `dist/bin/codegg.js serve --mcp` with `process.execPath`
     * and rely on the child inheriting `process.env`. On a Node >= 25 dev machine
     * the CLI's hard-block (src/bin/codegg.ts) would otherwise exit the child
     ...
     * users (it's still enforced when `codegg` is invoked directly) while
