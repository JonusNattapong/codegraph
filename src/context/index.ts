...
export class ContextBuilder {
  ...
  private buildLowConfidenceNote(entryPoints: Node[]): string {
    ...
    const dirLine = dirs.length
      ? `\n- \`codegg_files\` a likely area: ${dirs.map(d => `\`${d}\``).join(', ')}`
      : '';
    return `\n\n${LOW_CONFIDENCE_MARKER}\n\n`
      ...
      + '- `codegg_explore` with the **exact symbol names** you are after '
      + '(class / function / method names), or\n'
      + '- `codegg_search <name>` for one specific symbol'
      + dirLine
   ...
   * truncation is honest, and the agent can codegg_node the last hop to bridge.
   */
  private buildCallPathsSection(subgraph: Subgraph): string {
    ...
    const lines = [
      ...
      '',
      hasSynth
        ? '_Hops marked `[callback/event …]` are dynamic dispatch bridged by codegg (with the registration site); the rest are direct calls. codegg_node any symbol for its body._'
        : '_codegg_node any symbol above for its source + its own callers/callees._',
    ];
    return '\n' + lines.join('\n') + '\n';
  }
