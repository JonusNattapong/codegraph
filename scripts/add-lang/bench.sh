...
#   clone -> wipe+index (with the codegg on PATH) -> verify extraction ->
#   with/without retrieval A/B (reuses scripts/agent-eval/run-all.sh).
#
# Assumes the codegg dev build is already built + linked on PATH — the skill
...
# Env:   CORPUS   corpus dir (default /tmp/codegg-corpus, shared with agent-eval)
set -uo pipefail
...
CORPUS="${CORPUS:-/tmp/codegg-corpus}"
REPO="$CORPUS/$NAME"

command -v codegg >/dev/null || { echo "no codegg on PATH (build + ./scripts/local-install.sh first)"; exit 1; }

echo "==================== add-lang bench: $NAME ($LANG_TOKEN) ===================="
echo "codegg: $(command -v codegg) -> $(codegg --version 2>/dev/null || echo '?')"
...
echo "→ wiping .codegg and indexing"
rm -rf "$REPO/.codegg"
( cd "$REPO" && codegg init -i ) || { echo "indexing failed"; exit 1; }
