 ...
 *   - `CODEGG_ASCII=1`  -> ASCII (escape hatch for any terminal)
 *   - `CODEGG_UNICODE=1` -> Unicode (opt-in on Windows)
...
export function supportsUnicode(): boolean {
  if (process.env.CODEGG_ASCII === '1') return false;
  if (process.env.CODEGG_UNICODE === '1') return true;
