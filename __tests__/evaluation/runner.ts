...
import { CodeGG } from '../../src/index.js';
...
if (!fs.existsSync(path.join(resolvedPath, '.codegg', 'codegg.db'))) {
  console.error(`No .codegg/codegg.db found at ${resolvedPath}`);
  process.exit(1);
}

let codeggSha = 'unknown';
try {
  codeggSha = execSync('git rev-parse --short HEAD', { encoding: 'utf-8' }).trim();
} catch {}

console.log(`\nCodeGG Eval — ${path.basename(resolvedPath)}`);
console.log(`Codebase: ${resolvedPath}`);
console.log(`Commit:   ${codeggSha}`);
console.log(`Cases:    ${testCases.length}`);
console.log('');

async function run() {
  const cg = CodeGG.openSync(resolvedPath);
