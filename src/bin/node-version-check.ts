 ...
 * reliably crashes CodeGG with `Fatal process out of memory: Zone`
...
/**
 * Build the bordered banner shown when CodeGG detects an
...
export function buildNode25BlockBanner(nodeVersion: string): string {
  ...
  return [
    sep,
    `[CodeGG] Unsupported Node.js version: ${nodeVersion}`,
    sep,
    'Node.js 25.x has a V8 WASM JIT (turboshaft) Zone allocator bug that',
    'crashes with `Fatal process out of memory: Zone` when CodeGG',
    'compiles tree-sitter grammars. CodeGG WILL crash on this Node',
    'version mid-indexing. See https://github.com/jonusnattapong/codegg/issues/81',
    '',
    ...
    '  CODEGG_ALLOW_UNSAFE_NODE=1 codegg ...',
    sep,
  ].join('\n');
}
 ...
 * package.json. Below this, CodeGG relies on language features / native APIs
...
/**
 * Build the bordered banner shown when CodeGG detects a Node.js major below
...
export function buildNodeTooOldBanner(nodeVersion: string): string {
  ...
  return [
    ...
    `[CodeGG] Unsupported Node.js version: ${nodeVersion}`,
    sep,
    `CodeGG requires Node.js ${MIN_NODE_MAJOR} or newer. Older versions lack`,
    'language features and native APIs CodeGG depends on, and are not',
    'tested or supported.',
    '',
    'Fix: install Node.js 22 LTS:',
    '  nvm install 22 && nvm use 22                          # nvm',
