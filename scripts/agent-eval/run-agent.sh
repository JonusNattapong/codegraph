#!/usr/bin/env bash
# Headless Claude Code run against a repo with codegg MCP, capturing the
...
# Env: AGENT_EVAL_OUT (default /tmp/agent-eval), CG_BIN (codegg dist binary)
set -uo pipefail

REPO="$1"; LABEL="$2"; PROMPT="$3"
CG_BIN="${CG_BIN:-$(command -v codegg || echo /usr/local/bin/codegg)}"
