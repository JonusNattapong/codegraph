#!/usr/bin/env bash
# Re-run the README "Benchmark Results" A/B (with vs without codegg) on the
...
# $CORPUS (default /tmp/codegg-corpus) by the build under test.
set -uo pipefail
H="$(cd "$(dirname "$0")" && pwd)"
C="${CORPUS:-/tmp/codegg-corpus}"
RUNS="${RUNS:-4}"
ROWS=(
