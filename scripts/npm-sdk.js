'use strict';
//
// Programmatic / embedded SDK entry for @jonusnattapong/codegg (issue #354).
//
...
// API — `require("@jonusnattapong/codegg")` returning the CodeGraph class et al.
//
// The compiled library + its production dependencies (web-tree-sitter,
// tree-sitter-wasms, …) ship INSIDE the per-platform bundle, at
//   @jonusnattapong/codegg-<platform>-<arch>/lib/dist/index.js
...
function resolveLibrary() {
  ...
  throw new Error(
    'codegg: the programmatic API is unavailable because the platform bundle\n' +
    ...
    '      npm i @jonusnattapong/codegg --registry=https://registry.npmjs.org\n' +
    '  - or run the CLI once (e.g. `npx @jonusnattapong/codegg status`) to\n' +
    '    self-heal the bundle into ~/.codegg, then require() will find it.'
  );
}
...
function cachedLibrary() {
  try {
    ...
    var base = process.env.CODEGG_INSTALL_DIR || path.join(os.homedir(), '.codegg');
