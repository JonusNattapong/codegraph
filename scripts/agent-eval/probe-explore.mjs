...
// without a full agent run. Usage: node probe-explore.mjs <repo-with-.codegg> "<query>"
...
if (typeof CodeGG?.openSync !== 'function') {
  console.error('could not resolve CodeGG.openSync; index keys:', Object.keys(idx), 'default keys:', idx.default && Object.keys(idx.default));
  process.exit(2);
}
...
const cg = CodeGG.openSync(repo);
const h = new ToolHandler(cg);
const res = await h.execute('codegg_explore', { query });
