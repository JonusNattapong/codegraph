...
#
# Arms vary (exposed codegg tools, trace-first steering). Tools are trimmed
# SERVER-SIDE via CODEGG_MCP_TOOLS in the MCP config's `env` block, so an
...
CG_BIN="${CG_BIN:-$(command -v codegg)}"
OUT="${ARMS_OUT:-/tmp/arms}/$(basename "$REPO")"
mkdir -p "$OUT"
[ -n "$CG_BIN" ] || { echo "no codegg binary (set CG_BIN)"; exit 1; }
[ -d "$REPO/.codegg" ] || { echo "no .codegg index at $REPO"; exit 1; }

STEER='Flow questions ("how does X reach/become Y", "trace the flow", request to handler, state to render): call codegg_trace(from,to) FIRST — one call returns the whole path. Use codegg_context/search only to locate the two endpoint symbols if you do not know them. Do NOT reconstruct the path with repeated search/callers/explore.'
...
if [ -n "$TOOLS" ]; then
  cat > "$CFG" <<JSON
{"mcpServers":{"codegg":{"command":"$CG_BIN","args":["serve","--mcp","--path","$REPO"],"env":{"CODEGG_MCP_TOOLS":"$TOOLS"}}}}
JSON
else
  cat > "$CFG" <<JSON
{"mcpServers":{"codegg":{"command":"$CG_BIN","args":["serve","--mcp","--path","$REPO"]}}}
JSON
fi
