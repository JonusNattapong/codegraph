...
describe('MCP input size limits', () => {
  ...
  beforeEach(async () => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'codegg-mcp-limits-'));
    fs.mkdirSync(path.join(tempDir, 'src'), { recursive: true });
    fs.writeFileSync(
      path.join(tempDir, 'src', 'a.ts'),
      `export function alpha(): number { return 1; }\n`
    );
    cg = await CodeGG.init(tempDir, {
  ...
  it('accepts a normal-sized query', async () => {
    const result = await handler.execute('codegg_search', { query: 'alpha' });
    expect(result.isError).toBeFalsy();
  });

  it('rejects an oversize query on codegg_search', async () => {
    const huge = 'a'.repeat(20_000);
    const result = await handler.execute('codegg_search', { query: huge });
    expect(result.isError).toBe(true);
    expect(result.content[0]!.text).toMatch(/maximum length/i);
  });

  it('rejects an oversize query on codegg_explore', async () => {
    const huge = 'b'.repeat(50_000);
    const result = await handler.execute('codegg_explore', { query: huge });
    expect(result.isError).toBe(true);
    expect(result.content[0]!.text).toMatch(/maximum length/i);
  });

  it('rejects an oversize symbol on codegg_callers', async () => {
    const huge = 'c'.repeat(15_000);
    const result = await handler.execute('codegg_callers', { symbol: huge });
    expect(result.isError).toBe(true);
    expect(result.content[0]!.text).toMatch(/maximum length/i);
  });

  it('rejects an oversize symbol on codegg_impact', async () => {
    const huge = 'd'.repeat(11_000);
    const result = await handler.execute('codegg_impact', { symbol: huge });
    expect(result.isError).toBe(true);
    expect(result.content[0]!.text).toMatch(/maximum length/i);
  });
  ...
  it('rejects an oversize path filter on codegg_files', async () => {
    const hugePath = 'src/' + 'y'.repeat(5_000);
    const result = await handler.execute('codegg_files', { path: hugePath });
    expect(result.isError).toBe(true);
    expect(result.content[0]!.text).toMatch(/path/);
  });

  it('rejects an oversize glob pattern on codegg_files', async () => {
    const hugePattern = '*'.repeat(5_000);
    const result = await handler.execute('codegg_files', { pattern: hugePattern });
    expect(result.isError).toBe(true);
