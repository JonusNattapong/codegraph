 ...
 *
 *   mcp_servers.codegg -> `codegg serve --mcp`
 *   platform_toolsets.cli -> `mcp-codegg`
 *
 * The second entry matters because Hermes CLI profiles often enable an
 * explicit `platform_toolsets.cli` list. Without `mcp-codegg` in that
...
class HermesTarget implements AgentTarget {
  ...
  detect(loc: Location): DetectionResult {
    ...
    return {
      installed,
      alreadyConfigured: hasCodeGGMcpServer(content),
      configPath: file,
    };
  }
  ...
  uninstall(loc: Location): WriteResult {
    ...
    const after = removeCodeGGToolset(removeCodeGGMcpServer(before));
    if (after === before) {
      return { files: [{ path: file, action: 'not-found' }] };
    }
...
function writeHermesConfig(): WriteResult['files'][number] {
  ...
  const before = readText(file);
  const afterMcp = upsertCodeGGMcpServer(before);
  const after = upsertCodeGGToolset(afterMcp);
...
function renderCodeGGMcpChild(): string[] {
  return [
    '  codegg:',
    '    command: codegg',
    '    args:',
...
function renderCodeGGMcpBlock(): string[] {
  return ['mcp_servers:', ...renderCodeGGMcpChild()];
}

function hasCodeGGMcpServer(content: string): boolean {
  const lines = splitLines(content);
  const parent = topLevelRange(lines, 'mcp_servers');
  return !!parent && !!childRange(lines, parent, 'codegg');
}

function upsertCodeGGMcpServer(content: string): string {
  const lines = splitLines(content);
  const parent = topLevelRange(lines, 'mcp_servers');
  const child = parent ? childRange(lines, parent, 'codegg') : null;
  const replacement = renderCodeGGMcpChild();

  if (!parent) {
    if (lines.length > 0 && lines[lines.length - 1] === '') lines.pop();
    if (lines.length > 0) lines.push('');
    lines.push(...renderCodeGGMcpBlock());
...
function removeCodeGGMcpServer(content: string): string {
  const lines = splitLines(content);
  const parent = topLevelRange(lines, 'mcp_servers');
  const child = parent ? childRange(lines, parent, 'codegg') : null;
...
function upsertCodeGGToolset(content: string): string {
  const lines = splitLines(content);
  ...
  if (!parent) {
    if (lines.length > 0 && lines[lines.length - 1] === '') lines.pop();
    if (lines.length > 0) lines.push('');
    lines.push('platform_toolsets:', '  cli:', '    - hermes-cli', '    - mcp-codegg');
    return joinLines(lines);
  }

  if (!cli) {
    lines.splice(parent.end, 0, '  cli:', '    - hermes-cli', '    - mcp-codegg');
    return joinLines(lines);
  }

  const hasEntry = lines
    .slice(cli.start + 1, cli.end)
    .some((line) => line.trim() === '- mcp-codegg');
  if (hasEntry) return joinLines(lines);

  lines.splice(cli.end, 0, `${cli.itemIndent}- mcp-codegg`);
  return joinLines(lines);
}

function removeCodeGGToolset(content: string): string {
  const lines = splitLines(content);
  const parent = topLevelRange(lines, 'platform_toolsets');
  const cli = parent ? listChildBlock(lines, parent, 'cli') : null;
