...
/**
 * Whether a file is one CodeGG can parse, based purely on its extension.
...
export async function loadGrammarsForLanguages(languages: Language[]): Promise<void> {
  ...
  for (const lang of toLoad) {
    ...
    try {
      ...
      console.warn(`[CodeGG] Failed to load ${lang} grammar — parsing will be unavailable: ${message}`);
