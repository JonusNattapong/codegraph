 ...
 * @example
 * ```typescript
 * import { Mutex, processInBatches, MemoryMonitor, validatePathWithinRoot } from 'codegg';
 *
  ...
  acquire(): void {
    ...
    if (fs.existsSync(this.lockPath)) {
      try {
        ...
        if (lockAge < FileLock.STALE_TIMEOUT_MS && !isNaN(pid) && this.isProcessAlive(pid)) {
          throw new Error(
            `CodeGG database is locked by another process (PID ${pid}). ` +
            `If this is stale, run 'codegg unlock' or delete ${this.lockPath}`
          );
        }
    ...
    try {
      ...
      if (err.code === 'EEXIST') {
        ...
        throw new Error(
          'CodeGG database is locked by another process. ' +
          `If this is stale, run 'codegg unlock' or delete ${this.lockPath}`
        );
      }
      throw err;
