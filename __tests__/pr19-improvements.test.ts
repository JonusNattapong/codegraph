...
function createTempDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'codegg-pr19-test-'));
}
...
// =============================================================================
// Graph Traversal 'both' Direction Fix
// (requires better-sqlite3 - will use CodeGG integration)
// =============================================================================
...
describe('Graph Traversal Both Direction', () => {
  ...
  it.skipIf(!HAS_SQLITE)('should traverse both directions from a node', async () => {
    const CodeGG = (await import('../src/index')).default;
...
`);
    ...
    const cg = CodeGG.initSync(testDir, {
...
describe('Database Layer Improvements', () => {
  ...
  it.skipIf(!HAS_SQLITE)('should support batch insert of unresolved refs', async () => {
    ...
    const dbPath = path.join(testDir, 'codegg.db');
...
`);
      ...
      const cg = CodeGG.initSync(tmpDir, {
        config: { include: ['src/**/*.ts'], exclude: [] },
      });
      await cg.indexAll();
...
`);
      ...
      // note) so codegg_node can hand back every overload and the agent never
...
describe('CLI uninit', () => {
  ...
  it.skipIf(!HAS_SQLITE)('should uninitialize a project via CodeGG.uninitialize()', async () => {
    const CodeGG = (await import('../src/index')).default;

    // Initialize
    const cg = CodeGG.initSync(testDir);
    expect(CodeGG.isInitialized(testDir)).toBe(true);

    // Uninitialize
    cg.uninitialize();

    // .codegg directory should be removed
    expect(CodeGG.isInitialized(testDir)).toBe(false);
  });
});
