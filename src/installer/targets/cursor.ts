 ...
 *     as Claude.
 *   - Instructions to `./.cursor/rules/codegg.mdc` (project-local
 ...
 * `workspaceFolders` in the MCP initialize call. The codegg MCP
 * server's `process.cwd()` fallback therefore misses the workspace's
 * `.codegg/` and reports "not initialized" on every tool call.
 *
...
function rulesPath(): string {
  return path.join(process.cwd(), '.cursor', 'rules', 'codegg.mdc');
}
...
class CursorTarget implements AgentTarget {
  ...
  install(loc: Location, _opts: InstallOptions): WriteResult {
    ...
    // We no longer write `.cursor/rules/codegg.mdc` — the codegg
  ...
  printConfig(loc: Location): string {
    ...
    const snippet = JSON.stringify({ mcpServers: { codegg: buildCursorMcpConfig(loc) } }, null, 2);
...
/**
 * Build the codegg MCP-server config for Cursor at the given
 ...
 *
 * Unlike the shared CLAUDE.md / AGENTS.md files (where codegg owns
 * only a marker-delimited section), `.cursor/rules/codegg.mdc` is a
 ...
 * block but leave the orphaned `description: CodeGG ...` frontmatter
 * behind, so the file lingers and still "mentions" codegg.
 *
...
function removeRulesEntry(): WriteResult['files'][number] {
  ...
  const startIdx = content.indexOf(CODEGG_SECTION_START);
  const endIdx = content.indexOf(CODEGG_SECTION_END);

  // Our marked block is present — strip it, then decide what's left.
  if (startIdx !== -1 && endIdx > startIdx) {
    const before = content.substring(0, startIdx).trimEnd();
    const after = content.substring(endIdx + CODEGG_SECTION_END.length).trimStart();
