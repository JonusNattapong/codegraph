...
describe('buildNode25BlockBanner', () => {
  ...
  it('documents the CODEGG_ALLOW_UNSAFE_NODE override', () => {
    const banner = buildNode25BlockBanner('25.7.0');
    expect(banner).toContain('CODEGG_ALLOW_UNSAFE_NODE=1');
  });

  it('links to issue #81 for the root-cause writeup', () => {
    expect(buildNode25BlockBanner('25.7.0')).toContain(
      'github.com/jonusnattapong/codegg/issues/81'
    );
  });
});
...
describe('buildNodeTooOldBanner', () => {
  ...
  it('documents the CODEGG_ALLOW_UNSAFE_NODE override', () => {
    expect(buildNodeTooOldBanner('18.0.0')).toContain('CODEGG_ALLOW_UNSAFE_NODE=1');
  });
});
