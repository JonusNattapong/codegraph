 ...
 * CODEGG_DOWNLOAD_BASE — no real network, no published release needed. The
...
const asset = `codegg-${target}.tar.gz`;
...
function writeLauncher(binDir: string): void {
  ...
  const p = path.join(binDir, 'codegg');
...
describe.skipIf(isWindows)('npm-shim launcher', () => {
  ...
  it('uses an already-cached bundle even when downloads are disabled', async () => {
    ...
    const r = await runShim(pkg, ['--probe-xyz'], {
      CODEGG_INSTALL_DIR: cache,
      CODEGG_NO_DOWNLOAD: '1',
    });
  ...
  it('prints actionable guidance and exits 1 when disabled with no bundle', async () => {
    ...
    const r = await runShim(pkg, ['--version'], {
      CODEGG_INSTALL_DIR: mkTmp('cache'),
      CODEGG_NO_DOWNLOAD: '1',
    });
...
describe.skipIf(!CAN_NET)('npm-shim download fallback (local HTTPS)', () => {
  ...
  beforeAll(async () => {
    ...
    // Build a fake bundle archive (codegg-<target>/bin/codegg), like a real release asset.
    const work = mkTmp('fixture');
    writeLauncher(path.join(work, `codegg-${target}`, 'bin'));
    const archive = path.join(work, asset);
    execSync(`tar -czf ${JSON.stringify(archive)} -C ${JSON.stringify(work)} codegg-${target}`);
  ...
  function netEnv(cache: string): Record<string, string> {
    return {
      CODEGG_INSTALL_DIR: cache,
      CODEGG_DOWNLOAD_BASE: `https://127.0.0.1:${port}`,
  ...
  it('downloads, verifies the checksum, extracts, and execs the bundle', async () => {
    ...
    expect(fs.existsSync(path.join(cache, 'bundles', `${target}-5.0.0-net`, 'bin', 'codegg'))).toBe(true);
  }, 20000);
