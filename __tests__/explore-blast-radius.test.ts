/**
 * codegg_explore blast-radius section.
 *
...
describe('codegg_explore — blast radius', () => {
  let testDir: string;
  let cg: CodeGG;
  let handler: ToolHandler;

  beforeEach(async () => {
    testDir = fs.mkdtempSync(path.join(os.tmpdir(), 'codegg-blast-'));
  ...
  it('lists dependents (locations only) and covering tests for an entry symbol', async () => {
    const res = await handler.execute('codegg_explore', { query: 'target' });
  ...
  it('omits symbols that have no dependents from the blast radius', async () => {
    const res = await handler.execute('codegg_explore', { query: 'lonelyLeaf' });
    const text = res.content[0].text;
