...
for (const line of lines) {
  ...
  if (ev.type === 'system' && ev.subtype === 'init') {
    initTools = (ev.tools || []).filter(t => /codegg/.test(t));
  }
  if (ev.type === 'assistant' && ev.message?.content) {
    for (const block of ev.message.content) {
      if (block.type === 'tool_use') {
        ...
        else if (/codegg/.test(block.name)) detail = ` ${JSON.stringify(block.input?.query ?? block.input?.task ?? block.input?.symbol ?? '').slice(0,60)}`;
...
console.log(`\n=== ${file.split('/').pop()} ===`);
console.log(`codegg tools exposed: ${initTools ? initTools.length : '?'}`);
