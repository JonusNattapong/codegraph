...
// optionalDependency: @jonusnattapong/codegg-<platform>-<arch>. npm installs
...
//   CODEGG_INSTALL_DIR=DIR   cache location (default: ~/.codegg)
...
var pkg = '@jonusnattapong/codegg-' + target;
var isWindows = process.platform === 'win32';
var REPO = 'jonusnattapong/codegg';

main().catch(function (e) {
  process.stderr.write('codegg: ' + (e && e.message ? e.message : String(e)) + '\n');
  process.exit(1);
});
...
async function main() {
  ...
  if (res.error) {
    process.stderr.write('codegg: ' + res.error.message + '\n');
    process.exit(1);
  }
...
function resolveInstalledBundle() {
  try {
    if (isWindows) {
      ...
      var entry = require.resolve(pkg + '/lib/dist/bin/codegg.js');
      return { command: nodeExe, args: liftoff(entry) };
    }
    return { command: require.resolve(pkg + '/bin/codegg'), args: process.argv.slice(2) };
  } catch (e) {
    return null;
  }
}
...
function launcherIn(dir) {
  if (isWindows) {
    ...
    var entry = path.join(dir, 'lib', 'dist', 'bin', 'codegg.js');
    if (fs.existsSync(nodeExe) && fs.existsSync(entry)) {
      return { command: nodeExe, args: liftoff(entry) };
    }
  } else {
    var launcher = path.join(dir, 'bin', 'codegg');
...
// avoid the Zone OOM on Node >= 22 (issues #293/#298). The unix bin/codegg
...
async function selfHealBundle() {
  ...
  var bundlesDir = path.join(process.env.CODEGG_INSTALL_DIR || path.join(os.homedir(), '.codegg'), 'bundles');
  ...
  var asset = 'codegg-' + target + (isWindows ? '.zip' : '.tar.gz');
  var base = process.env.CODEGRAPH_DOWNLOAD_BASE || ('https://github.com/' + REPO + '/releases/download');
  var url = base + '/v' + version + '/' + asset;

  process.stderr.write(
    'codegg: platform bundle missing (registry did not provide ' + pkg + ').\n' +
    'codegg: downloading ' + asset + ' from GitHub Releases (' + version + ')...\n'
  );

  // Stage inside bundlesDir so the final rename is on the same filesystem (atomic,
  // no EXDEV across tmpfs). Strip the archive's top-level codegg-<target>/ dir.
  ...
  process.stderr.write('codegg: bundle ready.\n');
  return ready;
}
...
function download(url, dest, redirectsLeft) {
  return new Promise(function (resolve, reject) {
    ...
    var req = https.get(url, { headers: { 'User-Agent': 'codegg-npm-shim' }, timeout: 30000 }, function (res) {
...
async function verifyChecksum(archivePath, asset, base, version) {
  ...
  process.stderr.write('codegg: checksum verified.\n');
}
...
function fail(reason) {
  process.stderr.write(
    'codegg: no prebuilt bundle for ' + target + '.\n' +
    (reason ? 'codegg: ' + reason + '\n' : '') +
    ...
    '      npm i -g @jonusnattapong/codegg --registry=https://registry.npmjs.org\n' +
