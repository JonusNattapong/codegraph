#!/usr/bin/env bash
#
# Build a self-contained CodeGG bundle: an official Node runtime + the
# compiled app + its production deps, so CodeGG runs with NO system Node and
...
#
# Output:
#   unix:    release/codegg-<target>.tar.gz   (launcher: bin/codegg)
#   windows: release/codegg-<target>.zip      (launcher: bin/codegg.cmd)
set -euo pipefail
...
if [ "$OSFAM" = "win32" ]; then
  ...
  printf '@"%%~dp0..\\node.exe" --liftoff-only "%%~dp0..\\lib\\dist\\bin\\codegg.js" %%*\r\n' \
    > "$STAGE/bin/codegg.cmd"
else
  cp "$NODE_BIN" "$STAGE/node"
  cat > "$STAGE/bin/codegg" <<'LAUNCH'
#!/bin/sh
# Resolve symlinks (e.g. the ~/.local/bin/codegg link install.sh creates) so
...
exec "$DIR/node" --liftoff-only "$DIR/lib/dist/bin/codegg.js" "$@"
LAUNCH
  chmod +x "$STAGE/bin/codegg"
fi

# 5. Archive (.zip for Windows, .tar.gz otherwise).
mkdir -p "$OUT"
if [ "$OSFAM" = "win32" ]; then
  ARCHIVE="$OUT/codegg-${TARGET}.zip"
  rm -f "$ARCHIVE"
  ( cd "$WORK" && zip -rqX "$ARCHIVE" "codegg-${TARGET}" )
else
  ARCHIVE="$OUT/codegg-${TARGET}.tar.gz"
  # --no-xattrs: don't embed macOS xattrs that make GNU tar warn on Linux.
  tar --no-xattrs -czf "$ARCHIVE" -C "$WORK" "codegg-${TARGET}"
fi
