...
describe('watchDisabledReason', () => {
  it('returns a reason when CODEGG_NO_WATCH=1', () => {
    const reason = watchDisabledReason('/home/me/project', {
      env: { CODEGG_NO_WATCH: '1' },
      isWsl: false,
    });
    expect(reason).toBeTruthy();
    expect(reason).toMatch(/CODEGG_NO_WATCH/);
  });
  ...
  it('CODEGG_FORCE_WATCH=1 overrides WSL auto-detect', () => {
    const reason = watchDisabledReason('/mnt/d/code/project', {
      env: { CODEGG_FORCE_WATCH: '1' },
      isWsl: true,
    });
    expect(reason).toBeNull();
  });

  it('CODEGG_NO_WATCH wins over CODEGG_FORCE_WATCH', () => {
    const reason = watchDisabledReason('/home/me/project', {
      env: { CODEGG_NO_WATCH: '1', CODEGG_FORCE_WATCH: '1' },
      isWsl: false,
    });
    expect(reason).toBeTruthy();
  });
...
describe('FileWatcher honors the watch policy', () => {
  ...
  afterEach(() => {
    delete process.env.CODEGG_NO_WATCH;
    if (testDir && fs.existsSync(testDir)) {
      fs.rmSync(testDir, { recursive: true, force: true });
    }
  });

  it('does not start when CODEGG_NO_WATCH=1', () => {
    testDir = fs.mkdtempSync(path.join(os.tmpdir(), 'codegg-nowatch-'));
