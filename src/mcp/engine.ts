 ...
 * the project's {@link CodeGG} instance, file watcher, and the
...
// Lazy-load the heavy CodeGG chain (sqlite + query/graph/context layers) OFF
...
export class MCPEngine {
  private cg: CodeGG | null = null;
  private toolHandler: ToolHandler;
  // Project root we resolved to. Null until `ensureInitialized` succeeds
  // (or null forever if no .codegg/ ever turned up — that's a valid
  ...
  /** Whether the default project's CodeGG is open. */
  hasDefaultCodeGG(): boolean {
    return this.toolHandler.hasDefaultCodeGG();
  }

  /**
   * Walk up from `searchFrom` to find the nearest `.codegg/` and open it.
  ...
  async ensureInitialized(searchFrom: string): Promise<void> {
    ...
    if (this.toolHandler.hasDefaultCodeGG()) return;
    if (this.initPromise) {
  ...
  retryInitializeSync(searchFrom: string): void {
    ...
    const resolvedRoot = findNearestCodeGGRoot(searchFrom);
    ...
    try {
      ...
      this.cg = loadCodeGG().openSync(resolvedRoot);
      this.projectPath = resolvedRoot;
      this.toolHandler.setDefaultCodeGG(this.cg);
      this.startWatching();
      this.catchUpSync();
    } catch {
  ...
  private async doInitialize(searchFrom: string): Promise<void> {
    ...
    const resolvedRoot = findNearestCodeGGRoot(searchFrom);
    if (!resolvedRoot) {
      // No .codegg/ above searchFrom. Sessions may still discover one later via roots/list
    ...
    this.projectPath = resolvedRoot;
    try {
      this.cg = await loadCodeGG().open(resolvedRoot);
      this.toolHandler.setDefaultCodeGG(this.cg);
      this.startWatching();
      this.catchUpSync();
    } catch (err) {
      const msg = err instanceof Error ? err.message : String(err);
      process.stderr.write(`[CodeGG MCP] Failed to open project at ${resolvedRoot}: ${msg}\n`);
    }
  }

  /**
   * Start file watching on the active CodeGG instance. Idempotent — the
  ...
  private startWatching(): void {
    ...
    if (disabledReason) {
      process.stderr.write(
        `[CodeGG MCP] File watcher disabled — ${disabledReason}. ` +
        `The graph will not auto-update; run \`codegg sync\` (or install the git sync hooks via \`codegg init\`) to refresh.\n`
      );
    ...
    const debounceMs = parseDebounceEnv(process.env.CODEGG_WATCH_DEBOUNCE_MS);
    if (debounceMs !== undefined) {
      process.stderr.write(`[CodeGG MCP] File watcher debounce: ${debounceMs}ms (CODEGG_WATCH_DEBOUNCE_MS)\n`);
    }
    ...
    const started = this.cg.watch({
      ...
      onSyncComplete: (result) => {
        if (result.filesChanged > 0) {
          process.stderr.write(
            `[CodeGG MCP] Auto-synced ${result.filesChanged} file(s) in ${result.durationMs}ms\n`
          );
        }
      },
      onSyncError: (err) => {
        process.stderr.write(`[CodeGG MCP] Auto-sync error: ${err.message}\n`);
      },
    });

    this.watcherStarted = true;
    if (started) {
      process.stderr.write('[CodeGG MCP] File watcher active — graph will auto-sync on changes\n');
    } else {
      process.stderr.write(
        '[CodeGG MCP] File watcher unavailable on this platform — run `codegg sync` to refresh the graph after changes.\n'
      );
    }
  }
  ...
  private catchUpSync(): void {
    ...
    const p = cg
      ...
      .then((result) => {
        ...
        if (changed > 0) {
          process.stderr.write(`[CodeGG MCP] Caught up ${changed} file(s) changed since last run\n`);
        }
      })
      .catch((err) => {
        const msg = err instanceof Error ? err.message : String(err);
        process.stderr.write(`[CodeGG MCP] Catch-up sync failed: ${msg}\n`);
      });
...
/**
 * Parse and clamp the CODEGG_WATCH_DEBOUNCE_MS env override.
 *
