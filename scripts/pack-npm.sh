...
#   codegg-<target>/   one per built bundle — the vendored Node + app, tagged
#                         with os/cpu so npm installs only the matching one.
#   main/                 the @colbymchenry/codegg shim package: a tiny bin
...
#
# Prereq: run build-bundle.sh for each target first (release/codegg-*.tar.gz).
...
shopt -s nullglob
archives=("$REL"/codegg-*.tar.gz "$REL"/codegg-*.zip)
...
for archive in "${archives[@]}"; do
  ...
  case "$fname" in
    *.tar.gz) base="${fname%.tar.gz}" ;;   # codegg-<target>
    *.zip)    base="${fname%.zip}" ;;
  esac
  target="${base#codegg-}"             # <target>, e.g. darwin-arm64 / win32-x64
  ...
  VERSION="$VERSION" SCOPE="$SCOPE" TARGET="$target" OSV="$os" ARCHV="$arch" NODEFILE="$nodefile" \
    node -e '
      ...
      fs.writeFileSync(process.argv[1], JSON.stringify({
        name: `${process.env.SCOPE}/codegg-${process.env.TARGET}`,
        version: process.env.VERSION,
        description: `CodeGG self-contained bundle for ${process.env.TARGET}`,
    ...
    ' "$pkgdir/package.json"
  targets+=("$target")
  echo "[pack-npm] ${SCOPE}/codegg-${target}@${VERSION}"
done
...
VERSION="$VERSION" SCOPE="$SCOPE" TARGETS="${targets[*]}" \
  node -e '
    ...
    for (const t of process.env.TARGETS.split(/\s+/).filter(Boolean))
      opt[`${process.env.SCOPE}/codegg-${t}`]=process.env.VERSION;
    fs.writeFileSync(process.argv[1], JSON.stringify({
      name: `${process.env.SCOPE}/codegg`,
      version: process.env.VERSION,
...
echo "[pack-npm] ${SCOPE}/codegg@${VERSION} (${#targets[@]} platform packages in optionalDependencies)"
