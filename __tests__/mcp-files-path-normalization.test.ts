/**
 * codegg_files path-filter normalization (#426)
 *
...
describe('codegg_files path normalization', () => {
  let tempDir: string;
  let cg: CodeGG;
  let handler: ToolHandler;

  beforeEach(async () => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'codegg-files-paths-'));
  ...
  async function listed(pathFilter: string | undefined): Promise<string> {
    const result = await handler.execute('codegg_files', {
