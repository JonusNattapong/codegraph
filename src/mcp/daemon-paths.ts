 ...
 *
 * One shared `codegg serve --mcp` daemon per project root means we need a
 ...
 *
 * Both live under `.codegg/` so the project-scoped uninstall (`codegg
...
import { getCodeGGDir } from '../directory';
...
export function getDaemonSocketPath(projectRoot: string): string {
  if (process.platform === 'win32') {
    return `\\\\.\\pipe\\codegg-${projectHash(projectRoot)}`;
  }
  const inProject = path.join(getCodeGGDir(projectRoot), 'daemon.sock');
  ...
  return path.join(os.tmpdir(), `codegg-${projectHash(projectRoot)}.sock`);
}

/** Absolute path to the daemon pid lockfile for `projectRoot`. */
export function getDaemonPidPath(projectRoot: string): string {
  return path.join(getCodeGGDir(projectRoot), 'daemon.pid');
}
