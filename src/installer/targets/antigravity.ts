 ...
 * was then auto-migrated by Antigravity, and re-ran `codegg install`
 * doesn't end up with stale codegg entries in two files.
 *
 ...
 * to get codegg to appear in the Customizations UI. We build the
 ...
 * tools live outside that, so a bare `codegg` command fails to spawn
 * even when `which codegg` resolves in the user's shell. We resolve
 * `codegg` to its absolute path on macOS at install time. (Linux GUI
 ...
 * still get a working MCP integration; the prefer-codegg-over-grep
...
/**
 * Resolve the on-disk path of the `codegg` binary so a Mac GUI app
 * launched from Dock/Finder (with a stripped PATH) can find it. Falls
 * back to the bare `codegg` name when:
 *
...
function resolveCodegraphCommand(): string {
  if (process.platform !== 'darwin') return 'codegg';
  try {
    const resolved = execSync('command -v codegg || which codegg', {
      encoding: 'utf-8',
...
/**
 * Build the codegg MCP-server entry for Antigravity. Distinct from
...
class AntigravityTarget implements AgentTarget {
  ...
  detect(loc: Location): DetectionResult {
    ...
    const alreadyConfigured = !!config.mcpServers?.codegg;
  ...
  install(loc: Location, _opts: InstallOptions): WriteResult {
    ...
    // wind up with two competing codegg configs.
  ...
  uninstall(loc: Location): WriteResult {
    ...
    // vice versa) — handles the migration-half-state case where codegg
  ...
  printConfig(loc: Location): string {
    ...
    const file = preferredMcpConfigPath();
    const snippet = JSON.stringify({ mcpServers: { codegg: buildAntigravityEntry() } }, null, 2);
    return `# Add to ${file}\n\n${snippet}\n`;
  }
...
function writeMcpEntry(): WriteResult['files'][number] {
  ...
  const before = existing.mcpServers?.codegg;
  ...
  existing.mcpServers.codegg = after;
  writeJsonFile(file, existing);
  return { path: file, action };
}

/**
 * Strip the codegg entry from the legacy `~/.gemini/antigravity/mcp_config.json`
 * if it's present AND we're writing to the unified path. Used by install
 * to migrate users who had codegg configured on the legacy path
...
function cleanupLegacyEntry(): WriteResult['files'][number] | null {
  ...
  if (!config.mcpServers?.codegg) return null;
  delete config.mcpServers.codegg;
...
function removeCodegraphFromFile(file: string): WriteResult['files'][number] {
  ...
  const config = readJsonFile(file);
  if (!config.mcpServers?.codegg) return { path: file, action: 'not-found' };
  delete config.mcpServers.codegg;
  if (Object.keys(config.mcpServers).length === 0) {
    delete config.mcpServers;
  }
