 ...
 *     `./.kiro/settings/mcp.json` (local). Standard `mcpServers.codegg`
 *     shape, same as Claude / Cursor / Gemini.
 *   - Instructions to `~/.kiro/steering/codegg.md` (global) or
 *     `./.kiro/steering/codegg.md` (local). Kiro's "steering" system
 *     loads every `*.md` file in the steering dir as agent context, so
 *     a dedicated `codegg.md` is the natural surface — we own the
...
function steeringPath(loc: Location): string {
  return path.join(configDir(loc), 'steering', 'codegg.md');
}
...
class KiroTarget implements AgentTarget {
  ...
  install(loc: Location, _opts: InstallOptions): WriteResult {
    ...
    // The steering doc is no longer written — the codegg usage
    // guidance ships in the MCP server's `initialize` response (issue
    // #529). Delete a `codegg.md` a previous install created so an
    // upgrade self-heals.
 ...
 * out of recognition we still remove it — codegg.md is a name we
