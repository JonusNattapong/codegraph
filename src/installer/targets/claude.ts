 ...
 * codegg entry out of it on install and strip it on uninstall.
...
class ClaudeCodeTarget implements AgentTarget {
  ...
  install(loc: Location, opts: InstallOptions): WriteResult {
    ...
    // versions wrote `codegg mark-dirty` / `sync-if-dirty` hooks to
    ...
    // 3. CLAUDE.md instructions — no longer written. The codegg
  ...
  uninstall(loc: Location): WriteResult {
    ...
    // 1b. Also strip the codegg entry from a legacy ./.claude.json
    ...
    if (Array.isArray(settings.permissions?.allow)) {
      ...
      settings.permissions.allow = settings.permissions.allow.filter(
        (p: string) => !p.startsWith('mcp__codegg__'),
      );
    ...
    // 3. Instructions — strip the legacy CodeGG block if present.
  ...
  printConfig(loc: Location): string {
    ...
    const snippet = JSON.stringify({ mcpServers: { codegg: getMcpServerConfig() } }, null, 2);
...
export function writeMcpEntry(loc: Location): WriteResult['files'][number] {
  ...
  // `codegg` key) is 'updated', not 'created' — we're adding an
...
/**
 * Strip the codegg entry from a legacy project-local
 * `./.claude.json` (written by pre-#207 installers, which Claude Code
 * never read). Surgical: only our `codegg` key is removed; sibling
...
function isLegacyCodegraphHookCommand(command: unknown): boolean {
  ...
  return (
    command.includes('codegg mark-dirty') ||
    command.includes('codegg sync-if-dirty')
  );
}

/**
 * Remove stale codegg auto-sync hooks from Claude `settings.json`.
 *
 ...
 * codegg command, so a settings.json with no legacy hooks is left
...
export function cleanupLegacyHooks(loc: Location): WriteResult['files'][number] {
  ...
  // we never restructure a settings.json that had no codegg hooks.
  for (const event of Object.keys(hooks)) {
    const groups = hooks[event];
    if (!Array.isArray(groups)) continue;
...
export function writePermissionsEntry(loc: Location): WriteResult['files'][number] {
  ...
  const want = getCodeGGPermissions();
...
/**
 * Strip the marker-delimited CodeGG block from CLAUDE.md if a prior
