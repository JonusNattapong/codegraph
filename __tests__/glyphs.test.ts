...
describe('supportsUnicode', () => {
  ...
  it('returns false on Windows by default (mojibake-prone consoles)', () => {
    withEnv({ CODEGG_ASCII: undefined, CODEGG_UNICODE: undefined, TERM: undefined }, () => {
      setPlatform('win32');
  ...
  it('returns false on Linux kernel console (TERM=linux)', () => {
    withEnv({ CODEGG_ASCII: undefined, CODEGG_UNICODE: undefined, TERM: 'linux' }, () => {
      setPlatform('linux');
      expect(supportsUnicode()).toBe(false);
    });
  });

  it('respects CODEGG_UNICODE=1 on Windows (opt-in escape hatch)', () => {
    withEnv({ CODEGG_UNICODE: '1', CODEGG_ASCII: undefined }, () => {
      setPlatform('win32');
      expect(supportsUnicode()).toBe(true);
    });
  });

  it('respects CODEGG_ASCII=1 on macOS (opt-out escape hatch)', () => {
    withEnv({ CODEGG_ASCII: '1', CODEGG_UNICODE: undefined }, () => {
      setPlatform('darwin');
      expect(supportsUnicode()).toBe(false);
    });
  });

  it('CODEGG_ASCII takes precedence over CODEGG_UNICODE', () => {
    withEnv({ CODEGG_ASCII: '1', CODEGG_UNICODE: '1' }, () => {
      setPlatform('darwin');
      expect(supportsUnicode()).toBe(false);
    });
  });
...
describe('getGlyphs', () => {
  ...
  it('returns ASCII glyphs on Windows', () => {
    withEnv({ CODEGG_ASCII: undefined, CODEGG_UNICODE: undefined }, () => {
      setPlatform('win32');
      const g = getGlyphs();
