 ...
 *  1. `CODEGG_NO_WATCH=1`    → off  (explicit opt-out always wins)
 *  2. `CODEGG_FORCE_WATCH=1` → on   (overrides auto-detection)
...
export function watchDisabledReason(projectRoot: string, probe: WatchProbe = {}): string | null {
  ...
  if (env.CODEGG_NO_WATCH === '1') {
    return 'CODEGG_NO_WATCH=1 is set';
  }
  if (env.CODEGG_FORCE_WATCH === '1') {
    return null;
  }
