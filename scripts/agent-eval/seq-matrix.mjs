...
//   1. Trace adoption — on a flow question, does the with-arm actually call codegg_trace?
...
const MD = new URL('../../docs/benchmarks/codegg-ab-matrix.md', import.meta.url).pathname;
...
function parse(file) {
  ...
  for (const l of lines) {
    ...
    if (ev.type === 'system' && ev.subtype === 'init') initCg = (ev.tools || []).filter(t => /codegg/.test(t)).length;
  ...
  const cg = calls.filter(c => c.name.includes('codegg'));
  const perTool = {};
...
console.log('\n=== PER-CELL: with-arm codegg sequence + payload (sorted by repo size) ===');
...
console.log('\n=== with-arm TOTAL codegg payload by repo-size tier ===');
...
console.log('\n=== codegg tool usage across all cells (n calls, avg payload/call) ===');
...
console.log(`total codegg calls=${wCalls} (avg ${(wCalls / cells.length).toFixed(1)}/cell)`);
