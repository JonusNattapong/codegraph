 ...
 * (`Fatal process out of memory: Zone`) that crashed `codegg index` on large
...
describe('buildRelaunchArgv', () => {
  it('places the wasm flags first, then the script and its args', () => {
    expect(buildRelaunchArgv('/x/codegg.js', ['index', '/repo'], [])).toEqual([
      '--liftoff-only',
      '/x/codegg.js',
      'index',
      '/repo',
  ...
  it('preserves other existing node flags without duplicating ours', () => {
    expect(
      buildRelaunchArgv('/x/codegg.js', ['status'], ['--liftoff-only', '--enable-source-maps'])
    ).toEqual(['--liftoff-only', '--enable-source-maps', '/x/codegg.js', 'status']);
  });
