...
Distributed as `@jonusnattapong/codegg` on npm; same binary serves as installer, indexer, and MCP server.
...
`src/installer/` is the entry point for `codegg install` (and the bare `codegg`/`npx @jonusnattapong/codegg` invocation). Architecture:
