 ...
 * @example
 * ```typescript
 * import { FileError, ParseError, setLogger, silentLogger } from 'codegg';
 *
...
/**
 * Base error class for all CodeGG errors.
 *
 * All CodeGG-specific errors extend this class, allowing you to catch
 * all CodeGG errors with a single catch block.
 *
 * @example
 * ```typescript
 * try {
 *   await cg.indexAll();
 * } catch (error) {
 *   if (error instanceof CodeGGError) {
 *     console.log(`CodeGG error [${error.code}]: ${error.message}`);
 *   }
 * }
 * ```
 */
export class CodeGGError extends Error {
 ...
 */
export class FileError extends CodeGGError {
 ...
 */
export class ParseError extends CodeGGError {
  readonly filePath: string;
 ...
 */
export class DatabaseError extends CodeGGError {
 ...
 */
export class SearchError extends CodeGGError {
 ...
 */
export class VectorError extends CodeGGError {
  constructor(message: string, operation: string, cause?: Error) {
 ...
 */
export class ConfigError extends CodeGGError {
...
/**
 * Simple logger for CodeGG operations
 *
...
export const defaultLogger: Logger = {
  debug(message: string, context?: Record<string, unknown>): void {
    if (process.env.CODEGG_DEBUG) {
      console.debug(`[CodeGG] ${message}`, context ?? '');
    }
  },
  warn(message: string, context?: Record<string, unknown>): void {
    console.warn(`[CodeGG] ${message}`, context ?? '');
  },
  error(message: string, context?: Record<string, unknown>): void {
    console.error(`[CodeGG] ${message}`, context ?? '');
  },
};
