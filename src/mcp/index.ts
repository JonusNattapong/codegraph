 ...
 * CodeGG MCP Server
 *
 * Model Context Protocol server that exposes CodeGG functionality
 ...
 *   behavior; used when the user opts out (`CODEGG_NO_DAEMON=1`), no
 *   `.codegg/` is reachable, or the daemon machinery fails for any reason.
 ...
 *   sharing one CodeGG + watcher + SQLite handle. Spawned on demand; never a
...
import { findNearestCodeGGRoot, getCodeGGDir } from '../directory';
 ...
 */
const DAEMON_INTERNAL_ENV = 'CODEGG_DAEMON_INTERNAL';
...
// are cheap. Paired with deferring the CodeGG load (engine.ts) off the bind
...
/** Whether `CODEGG_NO_DAEMON` was set to a truthy value. */
function daemonOptOutSet(): boolean {
  const raw = process.env.CODEGG_NO_DAEMON;
  if (!raw) return false;
 ...
 * `null` when no `.codegg/` is reachable from the candidate path — in
 * that case the caller must run in direct mode, since the daemon lockfile
 * and socket both live under `.codegg/`.
 *
...
function resolveDaemonRoot(explicitPath: string | null): string | null {
  ...
  const root = findNearestCodeGGRoot(candidate);
  if (!root) return null;
 ...
 * `.codegg/daemon.log`). Re-invokes the *same* CLI faithfully across dev and
...
function spawnDetachedDaemon(root: string): void {
  ...
  try {
    logFd = fs.openSync(path.join(getCodeGGDir(root), 'daemon.log'), 'a');
...
/**
 * MCP Server for CodeGG
 *
 * Implements the Model Context Protocol to expose CodeGG
...
export class MCPServer {
   ...
   *
   * Decision order:
   *   1. `CODEGG_NO_DAEMON=1` → direct mode (unchanged pre-#411 behavior).
   *   2. `CODEGG_DAEMON_INTERNAL=1` → we ARE the detached daemon; listen.
   *   3. No `.codegg/` reachable → direct mode (the daemon's lockfile and
   *      socket both live under `.codegg/`).
  ...
  async start(): Promise<void> {
    ...
    if (daemonOptOutSet()) {
      return this.startDirect('CODEGG_NO_DAEMON set');
    }
    ...
    if (!root) {
      ...
      return this.startDirect('no .codegg/ root found');
    }
    ...
    try {
      ...
      process.stderr.write(`[CodeGG MCP] Proxy path failed (${msg}); falling back to direct mode.\n`);
  ...
  private async startDirect(reason: string): Promise<void> {
    if (reason && process.env.CODEGG_MCP_DEBUG) {
      process.stderr.write(`[CodeGG MCP] Direct mode: ${reason}.\n`);
    }
   ...
   * `CODEGG_DAEMON_INTERNAL=1`). Arbitrate the O_EXCL lock, then either
  ...
  private async startDaemonProcess(): Promise<void> {
    ...
    for (let attempt = 0; attempt < TAKEOVER_MAX_RETRIES; attempt++) {
      ...
      if (existing && existing.pid > 0 && isProcessAlive(existing.pid)) {
        process.stderr.write(
          `[CodeGG daemon] Another daemon (pid ${existing.pid}) already holds the lock; exiting.\n`
        );
        process.exit(0);
      }
    ...
    process.stderr.write('[CodeGG daemon] Could not acquire the daemon lock; exiting.\n');
    process.exit(0);
  }
  ...
  private installPpidWatchdog(): void {
    ...
    const pollMs = parsePpidPollMs(process.env.CODEGG_PPID_POLL_MS);
    if (pollMs <= 0) return;
    this.ppidWatchdog = setInterval(() => {
      ...
      if (ppidChanged || hostGone) {
        ...
        process.stderr.write(
          `[CodeGG MCP] Parent process exited (${reason}); shutting down.\n`
        );
        this.stop();
      }
    }, pollMs);
...
export { CodeGGPackageVersion } from './version';
