#!/usr/bin/env bash
# With/without A/B (and optional interactive) eval for a codegg version on a
# repo. Codegraph is the ONLY variable: both arms launch claude with
# --strict-mcp-config — with = codegg-only MCP (pointed at $CG_BIN),
...
# Env:   CG_BIN          codegg binary (default: command -v codegg)
...
[ -n "$CG_BIN" ] || { echo "no codegg binary on PATH (set CG_BIN)"; exit 1; }
[ -d "$REPO/.codegg" ] || { echo "no .codegg index at $REPO — index it first"; exit 1; }
...
if [ "$MODE" = headless ] || [ "$MODE" = all ]; then
  headless "headless-with"    "$OUT/mcp-codegg.json"
  headless "headless-without" "$OUT/mcp-empty.json"
fi

if [ "$MODE" = tmux ] || [ "$MODE" = all ]; then
  echo "############################## INTERACTIVE [with] ##############################"
  CLAUDE_EXTRA_ARGS="--model opus --strict-mcp-config --mcp-config $OUT/mcp-codegg.json" \
