...
# Env: CORPUS  corpus dir (default: /tmp/codegg-corpus)
set -uo pipefail
...
REPO_ROOT="$(cd "$HARNESS/../.." && pwd)"     # codegg repo root
CORPUS="${CORPUS:-/tmp/codegg-corpus}"
REPO="$CORPUS/$NAME"
PKG="@colbymchenry/codegg"

echo "==================== CodeGG audit ===================="
echo "version=$VERSION  repo=$NAME  mode=$MODE  corpus=$CORPUS"
echo

# 1. Set the codegg version under test (mutates the global install).
...
ACTUAL="$(codegg --version 2>/dev/null || echo '?')"
echo "  codegg on PATH: $(command -v codegg) -> $ACTUAL"
...
echo "→ [3/4] wiping .codegg and re-indexing with $ACTUAL"
rm -rf "$REPO/.codegg"
...
if ( cd "$REPO_ROOT" && ./scripts/local-install.sh >/dev/null 2>&1 ); then
  echo "  global codegg restored to dev build"
else
