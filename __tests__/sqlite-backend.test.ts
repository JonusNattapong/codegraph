 ...
 * DatabaseConnection / CodeGG report it and come up in WAL.
 */
...
describe('DatabaseConnection — backend reporting', () => {
  ...
  beforeEach(() => {
    dir = fs.mkdtempSync(path.join(os.tmpdir(), 'codegg-backend-'));
  });
  ...
  it('CodeGG.getBackend() delegates to the underlying DatabaseConnection', async () => {
    fs.writeFileSync(path.join(dir, 'x.ts'), `export function x(): void {}\n`);
    const cg = await CodeGG.init(dir, { index: true });
    try {
