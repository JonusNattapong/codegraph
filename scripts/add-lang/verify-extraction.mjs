#!/usr/bin/env node
// Sanity-check that codegg extracted REAL symbols (not just file/import nodes)
...
// Reads `codegg status <repo> --json` using whatever codegg is on PATH,
...
let status;
try {
  const out = execFileSync('codegg', ['status', repo, '--json'], { encoding: 'utf8' });
  status = JSON.parse(out);
} catch (e) {
  console.error(`[verify] could not read codegg status for ${repo}: ${e.message}`);
  process.exit(2);
}

// Kinds that prove the extractor mapped AST node types (everything except
// 'file' and 'import', which codegg creates structurally for any language).
