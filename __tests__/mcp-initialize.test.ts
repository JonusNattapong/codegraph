 ...
 * the MCP server was blocking the initialize response on CodeGG.open() and
...
function spawnServer(cwd: string): ChildProcessWithoutNullStreams {
  return spawn(process.execPath, [BIN, 'serve', '--mcp'], {
    ...
    // daemon (logging to .codegg/daemon.log, not the child's stderr); the
    ...
    env: { ...process.env, CODEGG_NO_DAEMON: '1' },
...
describe('MCP initialize handshake (issue #172)', () => {
  ...
  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'codegg-mcp-init-'));
  });
  ...
  it('responds to initialize quickly when no .codegg exists in cwd', async () => {
  ...
  it('sends initialize response BEFORE tryInitializeDefault finishes', async () => {
    // Seed a real .codegg so the server's tryInitializeDefault path runs
    // its full body: CodeGG.open() (which awaits initGrammars()) and then
    ...
    const json = JSON.parse(response.text);
    expect(json.id).toBe(0);
    expect(json.result.serverInfo.name).toBe('codegg');
  }, 20000);
